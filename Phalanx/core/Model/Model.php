...
class Model{
	...
	public function __construct($table_name, $use_memcache=USE_MEMCACHE, $memcache_seconds=MEMCACHE_SECONDS){
		$this->use_memcache = $use_memcache;
		$this->memcache_seconds = $memcache_seconds;
		$this->init($table_name);
	}
	
	public static function Factory($table_name, $use_memcache=USE_MEMCACHE, $memcache_seconds=MEMCACHE_SECONDS){
		return new self($table_name, $use_memcache, $memcache_seconds);	
	}
	...
	private function init($table_name){
		...
		$this->_db = MDB::Cursor($connection, $table_name);
	}
	...
	public function get(){
		...
		return $this->_db->fetchOne($this->use_memcache, $this->memcache_seconds);
	}
	...
	public function all(){
		if(!is_null($this->_whereStr))
			$this->_db->where($this->_whereStr);
		$data = $this->_db->fetchAll($this->use_memcache, $this->memcache_seconds);
