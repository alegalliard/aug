...
class Mysql{
	...
	public function fetchAll($use_memcache, $memcache_seconds){
	...
	public function insert($data){
		...
		return $status ? $this->insertID() : false;
	}
	...
	public function query($sql, $use_memcache, $seconds){
		
                #Hack to cache only the SELECTS
		if($use_memcache && ! preg_match('/^SELECT/i', $sql))
			$use_memcache = false;
		
		if($use_memcache){
			$memcache = new Memcache();
			$memcache_connection = $memcache->connect(MEMCACHE_SERVER, MEMCACHE_PORT);
			if(! $memcache_connection){
				
				$logger = Logger::getInstance('txt');
				$logger->file	= 'memcache_error_.'.date('ymd').'.txt';
				$logger->message(print_r($memcache, true));
				...
				if($data = $memcache->get($memcache_key)){
					$this->log("retrieving data from memcache", $data);
					return $data; 
				} else {
					$data = mysqli_query($this->conn, $sql);
					$data = $this->fetchObject($data);
					$memcache->set($memcache_key, $data, false, MEMCACHE_SECONDS);
				}
			}
		} else {
			$data = mysqli_query($this->conn, $sql);
