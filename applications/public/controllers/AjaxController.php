...
class AjaxController extends Controller {
    ...
    public function godfathered()
    {
        ...
        $html .= '<a href="'.HOST.'/apadrinhamento/cadastro/#'.$item->name.'" class="aug_purple"> Clique aqui para apadrinhar '.$item->name.'</a>.<br><br><br></p></article></div></li>';
    ...
    private function template($response) {
      ...
      $html = '<div class="col s12 m6 box-gatos p-0">
          ...
          if ($item->status == 2) {
           ...
           $html .= '<a href="'.HOST.'detalhes-gato/'.$item->id.'/'
                    .strtolower(str_replace(' ', '-', htmlentities($item->name))).
                    '" style="background-image: url('.MEDIA_DIR.'uploads/cats/'.$item->picture.');" class="cat-profile ';
    ...
    public function petz_paginated($response)
    {
        $page = $this->get->page;
        Phalanx::loadClasses('Cats');
        $response = Cats::get_paginated_petz($page);
        echo '<section id="gatinhos-para-adocao"><ul class="mural">';
        $this->template($response);
        echo '</ul></section>';
    }
